...
function SearchItemResult({ productData, filters }) {
  ...
  const handleSortByPriceValue = () => {
    let _value = `${filters.sortBy}:${filters.order}`
    if (_value !== 'price:asc' && _value !== 'price:desc') {
      _value = 'none'
    }
    return _value
  }
  ...
  return (
    <Styled.SearchItemResult>
      <Styled.SortBar>
        <Styled.SortBarLabel>Sắp xếp theo</Styled.SortBarLabel>
        <Styled.SortByOptions>
          <Styled.SortByOption
            onClick={() => sortBy('view')}
        ...
        <Styled.MiniPageController>
          <Styled.PageState>
            <Styled.PageStateCurrent>{pagination.page}</Styled.PageStateCurrent>{' '}
            /{' '}
            <Styled.PageStateTotal>
              {pagination.page_size}
            </Styled.PageStateTotal>
          </Styled.PageState>
          <Styled.PrevPageButton>
            <Button disabled={pagination.page === 1} onClick={getPrevPage}>
              <LeftArrowIcon />
            </Button>
          </Styled.PrevPageButton>
      ...
      {products && (
        <Styled.ProductList>
          {products.map(product => {
            return <MiniProduct key={product._id} product={product} />
          })}
        </Styled.ProductList>
      )}
...
const LeftArrowIcon = () => {
  return (
    <svg viewBox="0 0 7 11" className="icon-arrow-left-small">
      <path
